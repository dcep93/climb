...
app.get("/", function(req, res, next) {
    ...
    orm.select('gyms', {path: gym_path})
        ...
        .then(() => orm.select('walls', {gym_path}, {suffix: 'ORDER BY id DESC'}))
        .then((walls) => Object.assign(state.gym, {walls}))
        .then(() => orm.select('climbed_walls', {gym_path, user_id: req.session.user_id, active: true}, {columns: ['wall_id']}))
...
app.post("/new_wall", function(req, res, next) {
    ...
    orm.insert('walls', {gym_path, name, difficulty, location, date, setter, color, active})
        .then(() => res.sendStatus(200))
        .catch(next);
});
